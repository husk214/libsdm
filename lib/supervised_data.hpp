...
supervised_data<_Scalar, _Options>::supervised_data(
    const std::string &libsvm_format, const bool flag_rz) {
  sdm::load_libsvm_binary(x_, y_, libsvm_format, flag_rz);
  num_ins_ = x_.rows();
  num_fea_ = x_.cols();
}
...
template <typename _Scalar, int _Options>
int supervised_data<_Scalar, _Options>::get_num_ins(void) const {
  return num_ins_;
}

template <typename _Scalar, int _Options>
int supervised_data<_Scalar, _Options>::get_num_fea(void) const {
  return num_fea_;
}

template <typename _Scalar, int _Options>
int supervised_data<_Scalar, _Options>::get_num_err(
...
template <typename _Scalar, int _Options>
_Scalar
supervised_data<_Scalar, _Options>::get_err(const Eigen::VectorXd &w) const {
