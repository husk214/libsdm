...
template <typename ValueType1, int Major, typename Index>
bool split_libsvm_binary(Eigen::SparseMatrix<ValueType1, Major, Index> &x1,
                         Eigen::ArrayXd &y1,
                         Eigen::SparseMatrix<ValueType1, Major, Index> &x2,
  ...
  while (readline(fp) != nullptr) {
    ...
    if (num_ins == 0) {
      label1 = naive_atot<ValueType1>(p);
    }
    ++num_ins;
  }
  fclose(fp);
  ...
  for (int index = 0; readline(fp) != nullptr; ++index) {
    ...
    while (1) {
      ...
      if (d < k)
        d = k;
      tripletLists[wh].push_back(Tri(ns[wh], k, naive_atot<ValueType1>(val)));
    }
    ++ns[wh];
  }
  fclose(fp);
